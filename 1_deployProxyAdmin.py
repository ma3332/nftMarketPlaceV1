...
ProxyAdmin = open("ProxyAdmin.sol", "r").read()

# Compile solidity code
compiled_ProxyAdmin_sol = compile_standard(
    {
        "language": "Solidity",
        "sources": {"ProxyAdmin.sol": {"content": ProxyAdmin}},
        "settings": {
            "outputSelection": {
                "*": {
...
bytecode = compiled_ProxyAdmin_sol["contracts"]["ProxyAdmin.sol"]["ProxyAdmin"]["evm"][
    "bytecode"
]["object"]

# get abi (can be found from the compiled_code.json)
abi = compiled_ProxyAdmin_sol["contracts"]["ProxyAdmin.sol"]["ProxyAdmin"]["abi"]
...
proxy_admin_contract = w3.eth.contract(abi=abi, bytecode=bytecode)
...
print(f"Done! ProxyAdmin Contract deployed to {tx_receipt.contractAddress}")
